...
3. 使用以下任一方式提供账号数据：
   - 直接粘贴文本；
   - 选择一个或多个 `.txt` 文件；
   - 选择整个文件夹，系统会递归读取其中的 `.txt` 文件并忽略其他文件。
4. 文件内容会合并到预览文本框，可在提交前检查和编辑。每行格式：`邮箱----密码----client_id----refresh_token`。
5. 点击 **“确定”**。系统会显示新增、重复和无效账号数量，并列出被跳过的行及原因。

应用不限制单次选择的 TXT 文件数、账号行数或文本总长度。已有邮箱不会被覆盖；文件内重复、数据库已有账号和格式错误会分类跳过。
...
- **批量刷新 Token**：勾选账号后点击 **“批量刷新 Token”**。系统会用每个账号**当前已保存**的 Refresh Token 向 Microsoft 发起一次 refresh grant；成功时自动保存微软返回的轮换 Token，并标记为 `active`，失败则标记为 `error`。此操作**不读取邮件、不更新收件箱总数**。前端按每批 10 个顺序处理（关闭或刷新页面会中断尚未开始的批次）。
- **导入覆盖 Token**（可选）：若要从外部粘贴新 Token，使用批量栏的 **“导入覆盖 Token”**，每行 `邮箱----refresh_token`，须与当前选中账号一一对应。覆盖后会清除旧收件箱总数缓存，应再执行 **“批量测试连接”** 验证。
